...
if (annyang){
   ...
   var movements = phrases[0].replace(/\s/g, '');
   //var movement = movements.split(' ');
  ...
  // for (m in movement){
   if (movements) {
          switch(movements) {
            ...
            case 'drop': /*s*/   down=true; break;
            case 'down': /*s*/   down=true; break;

            case 'up': /*w*/    game._board.cur.rotate('right'); break;
            case 'up': /*w*/    game._board.cur.rotate('right'); break;
          }
        }
  ...
  $.widget('aerolab.blockrain', {
      ...
      this._board = {
        ...
        animate: function() {
          ...
          if( !this.paused && !this.gameover ) {
            ...
            if(left ||(this.holding.left && (now - this.holding.left) >= this.holdingThreshold )) {
              moved = true;
              this.cur.moveLeft();
              left=null;
            }
            ...
            if( right || this.holding.right &&(now - this.holding.right) >= this.holdingThreshold ) {
              moved = true;
              this.cur.moveRight();
              right=null;
            }
    ...
    _setupControls: function(enable) {
     ...
     var voiceControl= function(annyang){
       var movement='';
